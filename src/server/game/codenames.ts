    ...
    io.on('connection', (client: Socket) => {
        client.once('createGame', (options: CreateGameOptions, callback: (result: GameState | string) => void) => {
            try {
                ...
                callback("Une erreur s'est produite lors de la création du jeu.");
            }
        });
        client.once('joinGame', (options: JoinGameOptions, callback: (result: GameState | string) => void) => {
            try {
                ...
                callback("Une erreur s'est produite lors de la tentative de rejoindre la partie.");
            }
        });
        ...
        (function() {
            ...
            if (player != undefined) {
                ...
                if (game != undefined) {
                    ...
                    createLogger(game).info(`${player} s'est reconnecté.`);
                }
            }
        })();
...
function createPlayerOnSocket(socket: Socket, io: Server, name: string) {
    ...
    createLogger().info(chalk.green`${player} connecté et a été enregistré !`);
