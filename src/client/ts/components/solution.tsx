...
export const SolutionComponent: FunctionComponent = () => {
    ...
    return <div className="relative mx-6">
        <div className="absolute bg-gray-300 rounded-lg shadow-md inset-3 transform translate-y-5"/>
        <div className="relative bg-gray-100 rounded-lg shadow-md p-4">
            <div className="grid grid-cols-5 gap-2">
                { game.cards.map(card => {
                    return <div key={card.content} className={"w-5 h-5 rounded-sm " + getSolutionColour(card.colour)}/>
                }) }
            </div>
        </div>
    </div>
