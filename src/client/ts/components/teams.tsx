...
export const TeamComponent: FunctionComponent<TeamProps> = ({ players, colour, onSwitch }) => {
    ...
    return <div className={"relative m-auto transition-opacity " + (game.phase !== GamePhase.Round || game.inTurn === colour ? "opacity-100" : "opacity-50")}>
        <div className={"absolute rounded-lg shadow-md inset-2 transform translate-y-3 " + (isRedTeam ? "bg-red-400" : "bg-blue-400")}/>
        <div style={{ minWidth: "14rem" }} className="relative rounded-lg shadow-md bg-gray-100 px-8 py-8">
            ...
            { roundsWon > 0
                ? <div className="text-center">
                    { Array(roundsWon).fill('').map((v, i) => <span key={i} title="Jolie victoire !" className="text-yellow-400 mx-1 text-base"><i className="fas fa-trophy"/></span>) }
                </div>
                : null
            }
    ...
    + (i < cardsFound ? (isRedTeam ? "bg-red-400" : "bg-blue-400") : "bg-gray-300")}/>
                })}
            </div>
            }
