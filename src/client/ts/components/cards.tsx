...
export const CardComponent: FunctionComponent<CardProps> = ({ card, onClick }) => {
    ...
    return <div className="relative transform transition-transform hover:scale-110">
        <div className={"absolute rounded-lg shadow-md inset-2 transform translate-y-3 transition-colors " + underlayColour}/>
        <div className={"relative rounded-lg shadow-md transition-colors " + backgroundColour + (!card.isConsumed ? " hover:bg-gray-200" : "")} onClick={onClick}>
            { user.isGameMaster && !card.isConsumed ?
                <div className={"absolute w-4 h-4 top-2 left-2 rounded-full shadow-inner pointer-events-none " + getSolutionColour(card.colour)}/>
                : null }
